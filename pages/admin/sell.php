  ...
  <div class="table-responsive">
    <table class=" table-aom">
      ...
      <tr>
        ...
        <td><input type="text" name="kodeobat" class="full-width" id="kodeobat" placeholder="Pilih Barang.."  onClick="window.open('<?=base_url()?>pages/web/viewbarang.php','popuppage','width=700,toolbar=0,resizable=0,scrollbars=no,height=400,top=100,left=100');" required></td>
      </tr>
      <tr>
...
if(isset($_POST['button']))
{
	...
	if($rc==1)
	{
		...
		{
			$q=mysqli_query($koneksi, "Insert into obat_keluar (`tgl`,`kode_obat`,`jumlah`,`username`) values ('".$newDate."','".$_POST['kodeobat']."','".$_POST['qty']."','".$_SESSION['login_user']."')") or die(mysql_error());
			if($q)
			{
				...
				if($q3)
				{
					echo "<script>window.location='?cat=admin&page=sell'</script>";
				}
			}
		}else{
