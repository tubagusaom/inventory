...
if(isset($_GET['id']))
{
	$rs=mysql_query("Select * from lemari_uang where sha1(kode_lemari)='".$_GET['id']."'");
	$row=mysql_fetch_array($rs);
?>
<form name="form1" method="post" action="?cat=finance&page=lemariedit&id=<?php echo $_GET['id']; ?>&edit=1">
...
if(isset($_GET['edit']))
{
	...
	$rs=mysql_query("Update lemari_uang SET nama_lemari='".$_POST['namalemari']."' Where sha1(kode_lemari)='".$_GET['id']."'");
	if($rs)
	{
		echo "<script>window.location='?cat=finance&page=jenis'</script>";
	}
