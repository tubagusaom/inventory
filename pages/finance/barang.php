...
$sql_kode = mysql_query("SELECT kode_obat FROM data_finance order by kode_obat DESC LIMIT 1");
 if($sql_kode === FALSE) {
   die(mysql_error());
 }
      ...
      <tr>
        ...
        <td width="50%">
          <select name="kode_lemari" id="kode_lemari" >
     					...
     					<?php
     						$queryx = mysql_query("SELECT * FROM lemari_uang");
                 if($queryx === FALSE) {
                   die(mysql_error());
               	}
...
if(isset($_GET['act']))
{
	...
	$rs=mysql_query("Insert into data_finance (`kode_obat`,`nama_obat`,`kode_lemari`,`stts_obat`,`keterangan_barang`) values ('".$_POST['kode_obat']."','".$_POST['namaobat']."','".$_POST['kode_lemari']."','1','".$_POST['keterangan_barang']."')") or die(mysql_error());
	if($rs)
	{
...
{
	$ids=$_GET['id'];
	$ff=mysql_query("Delete from data_finance Where sha1(kode_obat)='".$ids."'");
	if($ff)
	{
		echo "<script>window.location='?cat=finance&page=barang'</script>";
	}
