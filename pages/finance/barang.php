...
$sql_kode = mysqli_query($koneksi, "SELECT kode_obat FROM data_finance order by kode_obat DESC LIMIT 1");
 if($sql_kode === FALSE) {
   die(mysqli_error());
 }
 ...
 $data_kode=mysqli_fetch_array($sql_kode);
	...
	if ($crop==0){
		$urutan="TLV-F-".date("y").date("m")."001";
	}

	elseif ($crop<9){
		$jum=$crop+1;
		$urutan="TLV-F-".date("y").date("m")."00".$jum;}

	elseif ($crop<99){
		$jum=$crop+1;
		$urutan="TLV-F-".date("y").date("m")."0".$jum;}

	elseif ($crop<999){
		$jum=$crop+1;
		$urutan="TLV-F-".date("y").date("m").$jum;}
...
?>
<form name="form1" method="post" action="?cat=finance&page=barang&act=1">
      ...
      <tr>
        ...
        <td width="50%">
          <select name="kode_lemari" id="kode_lemari" >
     					<option value="" style="color:darkblue; font-weight:700">- Pilih Jenis -</option>
     					<?php
     						$queryx = mysqli_query($koneksi, "SELECT * FROM lemari_uang");
                 if($queryx === FALSE) {
                   die(mysqli_error());
               	}
...
<span>
<?php
include("pages/finance/barangview.php");
?>
</span>
<?php
if(isset($_GET['act']))
{
	...
	$rs=mysqli_query($koneksi, "Insert into data_finance (`kode_obat`,`nama_obat`,`kode_lemari`,`stts_obat`,`keterangan_barang`) values ('".$_POST['kode_obat']."','".$_POST['namaobat']."','".$_POST['kode_lemari']."','1','".$_POST['keterangan_barang']."')") or die(mysql_error());
	if($rs)
	{
...
{
	$ids=$_GET['id'];
	$ff=mysqli_query($koneksi, "Delete from data_finance Where sha1(kode_obat)='".$ids."'");
	if($ff)
	{
		echo "<script>window.location='?cat=finance&page=barang'</script>";
	}
