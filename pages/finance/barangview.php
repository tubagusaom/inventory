...
<?php
	...
	$count_query   = mysql_query("SELECT COUNT(data_finance.kode_obat) AS numrows,data_finance.kode_obat, data_finance.nama_obat, data_finance.kode_lemari, data_persediaan.stok_tersedia
		FROM data_finance LEFT JOIN data_persediaan ON data_finance.kode_obat = data_persediaan.kode_obat" );
	...
	$query = mysql_query("SELECT
		data_finance.kode_obat,
		data_finance.nama_obat,
		data_finance.kode_lemari,
		data_finance.keterangan_barang,
		lemari_uang.nama_lemari
			FROM data_finance
				LEFT JOIN lemari_uang ON data_finance.kode_lemari = lemari_uang.kode_lemari
				WHERE data_finance.stts_obat = '1'
		-- LEFT JOIN data_persediaan ON data_finance.kode_obat = data_persediaan.kode_obat
		ORDER BY data_finance.kode_lemari LIMIT $offset,$per_hal");
...
<thead >
  ...
  <tr>
    ...
    <th style="background:#ece8e8">Kode Transaksi</th>
    <th style="background:#ece8e8">jenis</th>
    <th style="background:#ece8e8">Transaksi</th>
